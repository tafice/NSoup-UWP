...
namespace NSoup_UWP.Helper
{
    public class HttpConnection : IConnection
    {
        ...
        public Document Get()
        {
            req.Method(NSoup_UWP.Method.Get);
            ExecuteAsync();
            return res.Parse();
        }
        ...
        {
            req.Method(NSoup_UWP.Method.Post);
            ExecuteAsync();
            return res.Parse();
        }
        ...
        {
            res = (IResponse)await Helper.Response.ExecuteAsync(req);
            return res;
        }
    ...
    public abstract class ConnectionBase<T> : IConnectionBase<T> where T : IConnectionBase<T>
    {
        ...
        {
            return HasHeader(name) && Header(name).Equals(value, StringComparison.CurrentCultureIgnoreCase);
        }
    ...
    public class Response : ConnectionBase<IResponse>, IResponse
    {
        ...
        public static async Task<Response> ExecuteAsync(IRequest req)
        {
            return await ExecuteAsync(req, null);
        }

        public static async Task<Response> ExecuteAsync(IRequest req, IResponse previousResponse)
        {
            if (req == null)
            {
            ...
            if (req.Method() == NSoup_UWP.Method.Get && req.Data().Count > 0)
            {
            ...
            try
            {
                if (req.Method() == NSoup_UWP.Method.Post)
                {
                    conn.ContentType = "application/x-www-form-urlencoded";
                    await WritePostAsync(req.Data(), conn);
                }

                response = (HttpWebResponse) await conn.GetResponseAsync();
            }
            catch (WebException e)
            {
                ...
                if (response != null)
                {
                    return await ProcessResponseAsync(response, req, previousResponse);
                }
            }
        ...
        private static async Task<Response> ProcessResponseAsync(HttpWebResponse response, IRequest req, IResponse previousResponse)
        {
            ...
            if (needsRedirect && req.FollowRedirects())
            {
                ...
                return await ExecuteAsync(req, res);
            }
        ...
        private static HttpWebRequest CreateConnection(IRequest req)
        {
            ...
            {
                conn.Headers[HttpRequestHeader.UserAgent] = req.Header("User-Agent");
                req.RemoveHeader("User-Agent");
            }

            foreach (KeyValuePair<string, string> header in req.Headers())
            {
                conn.Headers[header.Key] = header.Value;
            }
        ...
        public void ProcessResponseHeaders(WebHeaderCollection resHeaders)
        {
            foreach (string name in resHeaders.AllKeys)
            {
                ...
                if (name.Equals("Set-Cookie", StringComparison.CurrentCultureIgnoreCase))
                {
        ...
        private static async Task WritePostAsync(ICollection<KeyVal> data, HttpWebRequest request)
        {
            Stream outputStream = await request.GetRequestStreamAsync();
            ...
            foreach (var keyVal in data)
            {
                ...
                sb.Append(WebUtility.UrlEncode(keyVal.Key()))
                    .Append('=')
                    .Append(WebUtility.UrlEncode(keyVal.Value()));
            }
        ...
        private static void SerialiseRequestUrl(IRequest req)
        {
            ...
            foreach (var keyVal in req.Data())
            {
                ...
                url.Append(WebUtility.UrlEncode(keyVal.Key()))
                    .Append('=').Append(WebUtility.UrlEncode(keyVal.Value()));
            }
